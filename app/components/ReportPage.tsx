...
export default function ReportPage({ config, fetchData, liveStations, onExportPdf, onExportExcel }: ReportPageProps) {
  ...
  const handleExportPdf = () => {
    if (onExportPdf) {
      onExportPdf({ stationIds: selectedIds, start: timeRange.start, end: timeRange.end });
    } else {
      console.log(`Export ${config.id} PDF for`, selectedIds);
    }
  };

  const handleExportExcel = () => {
    if (onExportExcel) {
      onExportExcel({ stationIds: selectedIds, start: timeRange.start, end: timeRange.end });
    } else {
      console.log(`Export ${config.id} Excel for`, selectedIds);
    }
  };
                ...
                <div className="flex flex-col gap-1.5 min-w-[132px]">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      ...
                      disabled={!!timeRangeError}
                      className={`inline-flex flex-1 items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] 2xl:text-xs font-semibold ${timeRangeError ? 'text-gray-400 bg-gray-50 border border-gray-200 cursor-not-allowed' : 'text-rose-700 bg-rose-50 border border-rose-200 hover:bg-rose-100'}`}
                    >
                      PDF
                    </button>
                    <button
                      ...
                      disabled={!!timeRangeError}
                      className={`inline-flex flex-1 items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] 2xl:text-xs font-semibold ${timeRangeError ? 'text-gray-400 bg-gray-50 border border-gray-200 cursor-not-allowed' : 'text-emerald-700 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100'}`}
                    >
                      Excel
                    </button>
                  </div>
